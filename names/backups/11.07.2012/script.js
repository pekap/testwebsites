...
// Add category
			...
			loader=$.post("index.php",{cname:new_cat},function(data){
				if(data.cname && data.cid && data.todo_array){
					...
					for ( var i in data.todo_array){
						s_menus="<tr name='"+data.cid+"'><td><a name='"+data.todo_array[i]['tid']+"' id='cat_menu_"+data.todo_array[i]['tid']+"_"+
	...
	var delete_cat_handler = function(event){
			...
			$(".el-cat>a[name="+cid+"]").text("input");
			$(".el-cat>a[name="+cid+"]").attr("name","1"); //вместо 1 должно быть номер input
	...
	$(".el-cat,.menus").live('mouseout',function(){
		$(".menus").hide();

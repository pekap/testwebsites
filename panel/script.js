$(document).ready(function(){
	...
	$("#deleted_todos").hide();
	$("#deleted_todos td.left").css("width","65px");
	$("#deleted_todos td.name-text").css("width","635px");
	$("#deleted_todos .properties").css("padding-left","75px");
	
	$("#name_field").focus();

	
	xyz=$.post("index.php",{inboxid:1},function(data){
	$("#r_b_"+data.in_id).remove();
	$(".left-menu div[name='"+data.in_id+"']").css({marginBottom:"20px"});
	},"JSON");
...
// Delete entry
	...
	var delete_handler = function(event){
			...
			if ($(this).hasClass("removed")){	
				...
				$(".thentr tr[name=thentr_"+id+"] td.left").prepend("<a id='name_ref_"+id+"' class='button refresh'>↑</a>");
				
				// changing of several properties back
				$("tr[name=thentr_"+id+"] td.left").css("width","65px");
				$("tr[name=thentr_"+id+"] td.name-text").css("width","635px");
				$("#name_"+id+" .properties").css("padding-left","75px");
				
				var sap=$("#name_"+id).html();
				$("#name_"+id).remove();
				sap="<div id='name_"+id+"' class='todo-entry'>"+sap+"</div>";
				$("#deleted_todos").prepend(sap);
				
				console.log($(this).attr("class"));
				// На сервере нужно заменить 1 на 0
				loader=$.post("index.php",{id:id,option:'ref',val:0},function(data){
					if(data){
					}
				});
			}
	...
	var refresh_handler = function(event){
			...
			$("#name_del_"+id).removeClass("removed");
			$("#name_ref_"+id).remove();
			
			// changing of several properties back
			$("tr[name=thentr_"+id+"] td.left").css("width","50px");
			$("tr[name=thentr_"+id+"] td.name-text").css("width","650px");
			$("#name_"+id+" .properties").css("padding-left","60px");
			
			var ccid=$("#name_"+id+" .el-cat>a").attr("name");
			var sap=$("#name_"+id).html();
			$("#name_"+id).remove();
			sap="<div id='name_"+id+"' class='todo-entry'>"+sap+"</div>";
			$("#names"+ccid).append(sap);
			
			console.log($(this).attr("class"));
			// На сервере нужно заменить 1 на 0
			
			event.preventDefault();
			loader=$.post("index.php",{id:id,option:'ref',val:1},function(data){
				if(data){
				}
			});
	};
	...
	$(".todo-entry .refresh").live('click',refresh_handler);
...
// deleting categories
	...
	var delete_cat_handler = function(event){
			...
			xyz=$.post("index.php",{inboxid:1},function(data){
				$(".el-cat>a[name="+cid+"]").attr("name",data.in_id); //вместо 1 должно быть номер inbox
				$("#names"+data.in_id+"").append($("#names"+cid).html());
			},"JSON");
			$("#names"+cid).remove();

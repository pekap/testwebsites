$(document).ready(function(){
	...
	var choose_categ_handler = function(){
		event.preventDefault();
	}
	
	$(".todo-entry .properties a.categ").live('click',choose_categ_handler);
		...
		if(name){
			...
			loader=$.post("index.php",{name:name,option:'add',cid:tabnum},function(data){
				if(data && data.id && data.name){
					$(nm).append("	<div id='name_"+data.id+"' class='todo-entry'>"+
									"<table>"+
										"<tr>"+
										...
										"</tr>"+
									"</table>"+
									"<div class='properties'><a class='categ'>"+data.cname+"</a></div>"+
								"</div>")
					field.value="";
				}
			},"JSON");
